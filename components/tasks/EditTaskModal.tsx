...
interface EditTaskModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: (taskId: string, updates: UpdateTaskInput) => Promise<void>;
  task: Task | null;
}
...
export function EditTaskModal({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await onSuccess(task.id, formData);
      setErrors({});
      onClose();
    } catch (error) {
  ...
  const handleChange = (field: keyof UpdateTaskInput, value: string) => {
    setFormData((prev: UpdateTaskInput) => ({ ...prev, [field]: value }));
    if (errors[field]) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={tTasks('edit.title')} size="lg">
